...
specific networks (see section [[#sec-user-sets]]).  In this case we say that
the IP addresses that belong to the specified networks are "interesting."
Similarly for top-level domains and bank cards.  Which artefacts gets shown in
the dashboard and stored in the database is user-configurable.
